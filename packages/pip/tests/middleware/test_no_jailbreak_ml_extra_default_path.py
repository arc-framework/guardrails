...
def test_top_level_factory_import_works_without_jailbreak_ml_extra(
    ...
    if sys.modules.get("transformers") is not None or sys.modules.get("torch") is not None:
        pytest.skip(
...
def test_default_pipeline_works_without_jailbreak_ml_extra(
    ...
    async def _run() -> None:
        ...
        assert any(f.entity_type.startswith("JAILBREAK_") for f in result.findings)
