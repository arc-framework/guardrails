...
def check_module() -> object:
    spec = importlib.util.spec_from_file_location("_check_public_surface_pip", TOOL_PATH)
    assert spec and spec.loader, f"could not load {TOOL_PATH}"
    module = importlib.util.module_from_spec(spec)
    sys.modules.setdefault("_check_public_surface_pip", module)
    spec.loader.exec_module(module)
    return module


def test_no_drift_for_arc_guard(check_module: object) -> None:
    errors = check_module.check_manifest()  # type: ignore[attr-defined]
    pip_errors = [e for e in errors if "[arc_guard]" in e]
    assert pip_errors == [], "\n".join(pip_errors)
