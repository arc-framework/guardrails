...
- Strict-shape CORS middleware backed by the new `ServiceSettings.dashboard_origins` allow-list. Wildcards / non-http(s) schemes / trailing slashes / path / query / fragment components are rejected at startup by the field validator. Methods restricted to `GET`, `POST`, and `OPTIONS`; credentials never flow cross-origin; named header allow-list (`Content-Type`, `Cache-Control`, `Last-Event-ID`, `X-Request-Id`); `Access-Control-Max-Age: 600`. Default-deny — empty list installs no middleware.
